/**
 * Il planning in PDF, un file per centro, disegnato alla build.
 *
 * ─── Perché un file e non la stampa del browser ───────────────────────────
 *
 * Perché questo PDF ha un lavoro preciso: essere mandato su WhatsApp e
 * attaccato in bacheca. La stampa del browser produce un risultato diverso su
 * ogni dispositivo — margini, intestazioni con l'URL, la data del sistema — e
 * su iPhone richiede quattro passaggi che molti non completano. Un file esiste
 * una volta, si chiama `macerata.pdf`, e chi lo riceve lo apre.
 *
 * ─── Perché disegnato a mano e non da un browser headless ─────────────────
 *
 * Generare un PDF stampando una pagina richiede Chromium nella build: due
 * minuti e trecento megabyte su Netlify, per un foglio. `pdf-lib` disegna
 * rettangoli e testo in un secondo, senza browser. In cambio il layout è
 * codice invece di CSS — accettabile per una griglia, che è la forma più
 * semplice che esista.
 *
 * ─── Resta allineato da sé ────────────────────────────────────────────────
 *
 * Legge le stesse `lezioni` della pagina e la stessa palette di
 * `src/data/categorie.ts`. Quando Alex cambia il planning dal CMS, il deploy
 * successivo rigenera il PDF: non esiste il caso del PDF vecchio accanto alla
 * griglia nuova, che è il difetto di ogni planning allegato a mano.
 */
import type { APIRoute, GetStaticPaths } from 'astro';
import { getCollection } from 'astro:content';
import { PDFDocument, StandardFonts, rgb, type PDFFont, type PDFPage } from 'pdf-lib';
import { lezioniValide, perSlot, slotDi, giorniDi, type Lezione } from '../../config/planning';
...
export const GET: APIRoute = async ({ props }) => {
  ...
  const apriPagina = (prima: boolean) => {
    ...
    if (prima) {
      ...
      pagina.drawText(`Planning ${dati.nome}`, {
        x: MARGINE + 62,
        y: y - 13,
        size: 13,
        font: grassetto,
      ...
      const sotto = [
        dati.planningAggiornatoIl
          ? `Aggiornato il ${new Intl.DateTimeFormat('it-IT', { day: 'numeric', month: 'long', year: 'numeric', timeZone: 'Europe/Rome' }).format(new Date(dati.planningAggiornatoIl))}`
          : '',
        'lumefitness.it',
      ]
        .filter(Boolean)
      ...
      if (dati.planningNota) {
        pagina.drawText(accorcia(dati.planningNota, normale, 7.5, gridW), {
          x: MARGINE,
          y: y - 41,
          size: 7.5,
          font: normale,
      ...
      pagina.drawText(`Planning ${dati.nome} — continua`, {
        x: MARGINE,
        y: y - 10,
        size: 9,
        font: grassetto,
  ...
  return new Response(new Blob([buffer], { type: 'application/pdf' }), {
    headers: {
      ...
      'Content-Disposition': `inline; filename="planning-${centro.id}.pdf"`,
    },
  });
};
